...
const ChatBot: React.FC<ChatBotProps> = ({ currentLang }: ChatBotProps) => {
  ...
  useEffect(() => {
    const welcomeText =
      WELCOME_MSGS[currentLang as string] || WELCOME_MSGS['en'] || WELCOME_MSGS['hi'];
    setMessages([
      {
        id: 'init',
        text: welcomeText,
  ...
  const placeholder =
    PLACEHOLDERS[currentLang as string] || PLACEHOLDERS['en'] || 'Ask about DBT…';
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-green-100 font-medium opacity-90">
                  {LANGUAGES.find((l) => l.code === (currentLang as string))?.name.split(
                    '('
                  )[0] || 'English'}{' '}
                  Mode
                </span>
